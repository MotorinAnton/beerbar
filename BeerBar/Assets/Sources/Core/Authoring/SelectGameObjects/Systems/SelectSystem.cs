...
namespace Core.Authoring.SelectGameObjects.Systems
{
    public partial class SelectSystem : SystemBase
    {
            ...
            Entities.WithAll<SelectObject>()
                .ForEach((Entity entity) =>
                {
                    if (EntityManager.HasComponent<UpgradeAndEventButtonUi>(entity))
                    {
                        ...
                        var upgradeAndEventButtonView = EntityManager.GetComponentObject<UpgradeAndEvenButtonUiView>(buttons);
            ...
            Entities.WithAll<DeselectObject>().WithNone<WaitTime>().ForEach(
                (Entity entity) =>
                {
                    if (EntityManager.HasComponent<UpgradeAndEventButtonUi>(entity))
                    {
                        ...
                        var upgradeAndEventButtonView = EntityManager.GetComponentObject<UpgradeAndEvenButtonUiView>(buttonsEntity);
        ...
        {
            var selectMaterialEntity = _selectMaterialQuery.ToEntityArray(Allocator.Temp)[0];
            var rendererSelectMaterial = EntityManager.GetComponentObject<SelectMaterial>(selectMaterialEntity).RendererObject;
