...
namespace Core.Authoring.ProductKeepers.Systems
{
    ...
    public partial class ProductKeeperMovementSystem : SystemBase
    {
        ...
        protected override void OnCreate()
        {
            ...
            _moveContainerProductKeeperQuery = moveContainerProductKeeperBuilder
                .WithAll<ProductKeeper, MoveContainerProductKeeper>().WithNone<MoveCharacter>().Build(this);

            using var moveWarehouseProductKeeperBuilder = new EntityQueryBuilder(Allocator.Temp);
            _moveWarehouseProductKeeperQuery = moveWarehouseProductKeeperBuilder
                .WithAll<ProductKeeper, MoveWarehouseProductKeeper>().WithNone<MoveCharacter>().Build(this);

            using var uploadProductKeeperBuilder = new EntityQueryBuilder(Allocator.Temp);
            _uploadProductKeeperQuery = uploadProductKeeperBuilder.WithAll<ProductKeeper, UploadProductKeeper>()
                .WithNone<WaitTime>().Build(this);

            using var unloadingProductKeeperBuilder = new EntityQueryBuilder(Allocator.Temp);
            _unloadingProductKeeperQuery = unloadingProductKeeperBuilder
                .WithAll<ProductKeeper, UploadProductKeeper, WaitTime>().Build(this);
            ...
            _spawnPointProductKeeperQuery =
                spawnPointProductKeeperBuilder.WithAll<SpawnPointProductKeeper>().Build(this);

            using var uploadPointSpillContainerBuilder = new EntityQueryBuilder(Allocator.Temp);
            _uploadPointSpillContainerQuery =
                uploadPointSpillContainerBuilder.WithAll<UploadPointSpillContainer>().Build(this);
        }
        ...
        private void MoveContainerProductKeeper()
        {
            ...
            foreach (var productKeeperEntity in moveContainerProductKeeperArray)
            {
                    ...
                    EntityManager.AddComponentData(productKeeperEntity,
                        new WaitTime
                        {
                            Current = AnimationUtilities.AnimationLength(animator,
                                ProductKeeperAnimationConstants.BoxAnim)
                        });
                ...
                {
                    EntityManager.GetComponentObject<ProductKeeperView>(productKeeperEntity).Value.PivotHand[0]
                        .gameObject.SetActive(true);

                    var orders = EntityManager.GetBuffer<OrderProductKeeper>(productKeeperEntity);
                    var orderPoint =
                        barmanContainerPoints.FirstOrDefault(point => point.Container == orders[0].Container);
                    ...
                    EntityManager.AddComponentData(productKeeperEntity,
                        new MoveCharacter { TargetPoint = orderPoint.Point.Position });
                    EntityManager.AddComponentData(productKeeperEntity,
                        new IndexOrderPoint { Value = orderPoint.Index });
                }
            }
        }
        ...
        private void UploadProductKeeper()
        {
            ...
            foreach (var productKeeperEntity in uploadProductKeeperArray)
            {
                ...
                var box = EntityManager.GetComponentObject<ProductKeeperView>(productKeeperEntity).Value.PivotHand[0]
                    .gameObject;
                box.SetActive(false);
        ...
        private void UnloadingProductKeeper()
        {
            ...
            foreach (var productKeeperEntity in unloadingProductKeeperArray)
            {
                    ...
                    !EntityManager.HasComponent<TweenProcessing>(orders[0].Container))
                {
                    ...
                    const float maxSpillYPosition = ProductKeeperAnimationConstants.MaxSpillYPosition;
                    const float durationMoveSpillY = ProductKeeperAnimationConstants.DurationMoveSpillY;
                    const float delayMoveSpillY = ProductKeeperAnimationConstants.DelayMoveSpillY;
                    ...
                    productView.Products[0][0].transform.DOLocalMoveY(pos.y, durationMoveSpillY)
                        .SetDelay(delayMoveSpillY)
                        .SetEase(Ease.Linear)
