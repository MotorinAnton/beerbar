    ...
    public partial class CreateDirtTableSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            Entities.WithAll<Table, DirtTableViewEntities>().ForEach(
                (Entity entity) => { CreateDirtTable(entity); }).WithoutBurst().WithStructuralChanges().Run();
        }
        ...
        private void CreateDirtTable(Entity entity)
        {
            ...
            if (dirtValue > dirtTableViewCount)
            {
                ...
                for (var i = dirtTableViewCount; i < dirtValue; i++)
                {
                    ...
                    var dirtView = Object.Instantiate(config.ProductConfig.Products[0].Prefabs[0],
                        spawnPointDirt.Position, spawnPointDirt.Rotation);
